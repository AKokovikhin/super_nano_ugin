...
def create_device_models():
    ...
    models_data = [
        ...
        ("Cisco Catalyst 2960", "Коммутатор", "48-портовый коммутатор"),  # ← ДОБАВЬТЕ ЭТУ СТРОКУ

        ("HiWatch DS-I114", "Камера", "Купольная IP-камера 4MP"),
        ("RVI-1NCT4030", "Камера", "Уличная IP-камера с ИК-подсветкой"),
        ("Secret Mini", "Камера", "Миниатюрная камера наблюдения"),  # ← ДОБАВЬТЕ ЭТУ СТРОКУ
        ("Hikvision DS-2CD2143G0", "Камера", "Уличная купольная камера 4MP"),  # ← ДОБАВЬТЕ ЭТУ СТРОКУ
        ...
        ("MikroTik cAP", "WiFi Точка доступа", "Компактная точка доступа"),  # ← ДОБАВЬТЕ ЭТУ СТРОКУ
    ]
    ...
    for name, type_name, description in models_data:
        ...
        if created:
            ...
            print_info(f"Модель уже существует: {name} ({type_name})")
